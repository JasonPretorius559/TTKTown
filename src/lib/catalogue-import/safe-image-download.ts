...
export async function downloadSourceImage(value: string, redirects = 0): Promise<Buffer> {
  ...
  const hosts = (process.env.CATALOGUE_IMAGE_HOSTS || "").split(",").map(value => value.trim().toLowerCase()).filter(Boolean);
  if (url.protocol !== "https:" || url.username || url.password || (url.port && url.port !== "443") || !hosts.includes(url.hostname)) throw new Error("Image host is not enabled");
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = request({ hostname: addresses[0], servername: url.hostname, path: url.pathname + url.search,
      method: "GET", headers: { Host: url.hostname, Accept: "image/jpeg,image/png,image/webp", "User-Agent": "TinkerTown-ImageImport/1.0" } }, response => {
