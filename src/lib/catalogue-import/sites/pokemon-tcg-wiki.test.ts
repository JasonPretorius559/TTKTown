...
import { parsePokemonCardTable,parsePokemonExpansionLinks,pokemonPageImages } from "./pokemon-tcg-wiki";
...
describe("Pokémon TCG Wiki parser",()=>{
  ...
  it("keeps card artwork addressable through normalized and redirected page titles",()=>{
    const images=pokemonPageImages({query:{
      normalized:[{from:"mr. mime (base set)",to:"Mr. mime (base set)"}],
      redirects:[{from:"Mr. mime (base set)",to:"Mr. Mime (Base Set)"}],
      pages:{"42":{title:"Mr. Mime (Base Set)",thumbnail:{source:"https://static.wikia.nocookie.net/pokemon/images/mr-mime.png"}}}
    }});
    expect(images.get("mr. mime (base set)")).toBe("https://static.wikia.nocookie.net/pokemon/images/mr-mime.png");
  });
});
