...
export async function POST(request:NextRequest){
  ...
  try{
    const result=await discoverFunkoFigures(runId,limit);const candidates=await importCandidateImages(result.candidates);
    return NextResponse.json({runId,source:"FUNKO",franchise:"Multi-franchise",queries:[result.sitemap],discovered:result.discovered,eligible:result.eligible,staged:candidates.length,candidates});
