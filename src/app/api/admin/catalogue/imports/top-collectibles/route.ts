...
export async function POST(request:NextRequest){
  ...
  const runId=randomUUID();try{await acquireImportSlot();const result=await discoverTopCollectibles(runId);const candidates=await importCandidateImages(result.candidates);const summary=await publishImport(user.uid,runId,candidates);return NextResponse.json({runId,source:"WEB",franchise:"Multi-franchise",queries:result.sources,discovered:result.discovered,staged:candidates.length,...summary,candidates})}catch(reason){return NextResponse.json({error:reason instanceof Error?reason.message:"Import failed"},{status:502})}
}
