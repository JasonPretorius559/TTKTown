...
type ImportResult={approved?:number;duplicates?:number;held?:number;error?:string;runId?:string;source?:CatalogueCandidate["source"];franchise?:string;queries?:string[];discovered?:number;candidates?:CatalogueCandidate[]};
...
const CATALOGUE_SOURCES:CatalogueSource[]=[
  {id:"funko",label:"Funko",logo:"/provider-logos/funko.svg",url:"/api/admin/catalogue/imports/funko",body:{limit:25},source:"FUNKO",franchise:"Multi-franchise",scope:"Collectible figure lines",note:"25 items per batch"},
  {id:"mattel",label:"Mattel",logo:"/provider-logos/mattel.svg",url:"/api/admin/catalogue/imports/providers",body:{provider:"MATTEL",limit:25},source:"MATTEL",franchise:"Multi-franchise",scope:"Collector brands and releases",note:"25 items per batch"},
  {id:"hot-wheels",label:"Hot Wheels Wiki",logo:"/provider-logos/hot-wheels.png",url:"/api/admin/catalogue/imports/providers",body:{provider:"HOT_WHEELS_WIKI",limit:25},source:"HOT_WHEELS_WIKI",franchise:"Hot Wheels",scope:"Community model catalogue",note:"25 items per batch",logoClass:"logo-wide"},
  {id:"pokemon",label:"Pokémon TCG Wiki",logo:"/provider-logos/pokemon-tcg.png",url:"/api/admin/catalogue/imports/providers",body:{provider:"POKEMON_TCG_WIKI",limit:25},source:"POKEMON_TCG_WIKI",franchise:"Pokémon Trading Card Game",scope:"Sets and trading cards",note:"25 items per batch",logoClass:"logo-tall"},
  {id:"gcd",label:"Grand Comics Database",logo:"/provider-logos/gcd.png",url:"/api/admin/catalogue/imports/providers",body:{provider:"GCD",limit:25},source:"GCD",franchise:"Multi-franchise",scope:"Selected comic issue lanes",note:"25 items per batch"},
  {id:"mcfarlane",label:"McFarlane Toys",logo:"/provider-logos/mcfarlane.png",url:"/api/admin/catalogue/imports/providers",body:{provider:"MCFARLANE",limit:10},source:"MCFARLANE",franchise:"Multi-franchise",scope:"Official figures and statues",note:"10 items per batch",logoClass:"logo-dark"},
...
export default function CatalogueImportsPage(){
  ...
  const importCatalogue=async(source:CatalogueSource)=>{
    ...
    try{
      setProgress(`Importing and checking ${source.label}…`);
      ...
      setMessage(`${source.label}: ${result.approved||0} approved, ${result.duplicates||0} duplicates, ${result.held||0} held safely. No individual approval required.`);
  ...
  return <div className="page catalogue-import-page">
    <PageHeader eyebrow="Admin Catalogue" title="Source Lookup Desk" description="Import compact product images and automatically publish records that pass all checks." action={<Link className="btn btn-outline" href="/admin">Back to admin</Link>}/>
    <section className="import-console source-console">
      <div><span className="eyebrow">On-demand catalogue</span><h2>Real pictures. Controlled growth.</h2><p>Each item gets one optimised picture, capped at 200 KB. Enabled sources pass through automated safety checks and duplicate detection. Storage budgets pause new uploads before capacity is exceeded.</p></div>
      <div className="import-principle"><ScanSearch/><span><strong>Automatic approval</strong><small>Uncertain records stay unpublished. No routine review queue.</small></span></div>
    </section>
