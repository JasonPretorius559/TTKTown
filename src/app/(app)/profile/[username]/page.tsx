...
export default function ProfilePage({params}:{params:Promise<{username:string}>}){
  ...
  return <div className="page"><DataState loading={profileState.loading} error={profileState.error} empty={!profile}>{profile&&<>
    ...
    <section className="section" id="posts"><div className="section-head"><h2>Recent posts</h2></div>{postsState.data.length?<div className="profile-posts">{postsState.data.map(post=><Link href={`/posts/${post.id}`} className="card" key={post.id}>{post.mediaType==="VIDEO"&&<div className="profile-video-label">Watch short - {Math.ceil(post.videoDuration||0)}s</div>}{post.image&&<Image src={post.image} alt="" width={220} height={180}/>}<p>{post.caption||"View post"}</p></Link>)}</div>:<EmptyState title="No Posts Yet" description="This collector has not shared a shelf update."/>}</section>
