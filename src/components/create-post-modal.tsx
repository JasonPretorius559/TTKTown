...
export function CreatePostModal({ open, onClose, user, initialMode=null }: { open:boolean; onClose:()=>void; user:User; initialMode?:Mode }) {
  ...
  return <div className="post-modal-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget)close()}}><div className="post-modal" role="dialog" aria-modal="true" aria-labelledby="create-post-title" ref={dialogRef}>
    <header className="post-modal-head"><button className="icon-btn" onClick={close} disabled={saving} aria-label="Close create post"><X size={19}/></button><h2 id="create-post-title">Create Post</h2><span/></header>
    ...
    <textarea ref={textareaRef} value={caption} disabled={saving} onChange={e=>setCaption(e.target.value)} maxLength={2000} placeholder="What’s new on your shelf?" aria-label="Post text"/>
    {mode==="video"&&<VideoUploader value={video} onChange={setVideo}/>}
    ...
    </div><footer className="post-modal-footer"><span aria-live="polite">{saving&&video?videoProgress:`${caption.length}/2,000`}</span><button className="btn btn-primary" onClick={publish} disabled={saving||(!caption.trim()&&!images.length&&!figureId&&!listingId&&!video)}>{saving?"Posting…":video?"Share short":"Post"}</button></footer>
  </div></div>;
}
